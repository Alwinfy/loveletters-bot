...
new Command(function(msg, serv, args) {
	if(util.devs.indexOf(msg.author.id) != -1)
...
new Command(function(msg, serv, args) {
	if(util.maySetOpts(msg)) {
		serv.set('welcome', args.join(' '));
		msg.channel.send('Welcome message is now ' + (serv.get('welcome').length ? `\`@USER ${serv.get('welcome')}\`!` : 'disabled.'));
	}
	else msg.channel.send('You may not modify the welcome message (need MANAGE_MESSAGES)!');
}, 'setmsg', 'sets the server\'s welcome message', false);
new Command(function(msg, serv, args) {
	if(util.maySetOpts(msg)) {
		serv.set('prefix', (args[0] + (/[a-zA-Z0-9]$/.test(args[0]) ? ' ' : '')) || '$');
		msg.channel.send(`Bot prefix is now \`${serv.get('prefix')}\`!`);
	}
	else msg.channel.send('You may not modify server prefices (need MANAGE_MESSAGES)!');
}, 'setprefix', 'sets the server\'s bot prefix', false);
new Command(function(msg, serv, args) {
	if(util.maySetOpts(msg)) {
		serv.set('replypfx', (args[0] + (/[a-zA-Z0-9]$/.test(args[0]) ? ' ' : '')) || '$');
		msg.channel.send(`Gameplay prefix is now \`${serv.get('replypfx')}\`!`);
	}
	else msg.channel.send('You may not modify server prefices (need MANAGE_MESSAGES)!');
}, 'setreply', 'sets the server\'s gameplay prefix', false);
new Command(function(msg, serv, args) {
